...
from launch.actions import DeclareLaunchArgument, SetEnvironmentVariable, SetLaunchConfiguration, GroupAction, Shutdown
...
def launch_setup(context, *args, **kwargs):
		...
		GroupAction(actions=[
			...
			Node(
				...
				parameters=[
					...
					{"results_file": parse_substitution("Results/D3/$(var method).csv")},
	...
	gaden = [
		...
		Node(
			...
			parameters=[
				...
				{"simulation_data_0" : parse_substitution("$(find-pkg-share pmfs_env)/$(var scenario)/gas_simulations/D3")},
	...
	basic_sim = Node(
		...
		parameters=[
			{"deltaTime": 0.1},
			{"speed": 5.0},
			{"worldFile": parse_substitution("$(find-pkg-share pmfs_env)/$(var scenario)/sim_start_living_room.yaml")}
			],
	)
