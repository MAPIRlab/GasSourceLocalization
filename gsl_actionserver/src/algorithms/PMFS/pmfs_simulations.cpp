...
void Simulations::updateSourceProbability(float refineFraction)
{
    ...
    std::vector<std::vector<VarianceCalculationData> > varianceCalculationData(grid->cells.size(), std::vector<VarianceCalculationData>(grid->cells[0].size()));
    ...
    while(scores.size()>0){
        ...
        std::sort(scores.begin(), scores.end(), [](SimulationResult result1, SimulationResult result2) {
    ...
    spdlog::info("Time ellapsed in simulation = {} s", Utils::Time::toSeconds(clock.now()-startT));
}
...
double Simulations::weightedDifference(const std::vector<std::vector<Cell> >& cells, const std::vector<std::vector<float> >& hitMap)
{
    ...
    for(int i =0; i<cells.size(); i++){
        for(int j =0; j<cells[0].size(); j++){
            ...
            double val =  Utils::lerp(1, (1- std::abs(measured-simulated)* grid->settings.simulation.sourceDiscriminationPower), cells[i][j].hitProbability.confidence) ;
            total *= val; 
...
void Simulations::moveFilament(Filament& filament, Vector2Int& indices, float deltaTime, float noiseSTDev)
{
    ...
    Utils::Vector2 newPos = filament.position + deltaTime * (grid->estimatedWindVectors[indices.x][indices.y] + 
...
bool Simulations::filamentIsOutside(Filament& filament){
    ...
    return (newIndices.x <0 || newIndices.x>=grid->cells.size() ||
        newIndices.y <0 || newIndices.y>=grid->cells[0].size());
}

void Simulations::simulateSourceInPosition(const SimulationSource& source, std::vector<std::vector<float> >& hitMap, bool warmup, int warmupLimit, int timesteps, float deltaTime, float noiseSTDev){
    ...
    //warm-up: we don't want to start recording frequency of hits until the shape of the plume has stabilized. Wait until a filament exits the environment through an outlet, or a maximum number of steps
    {
        bool stable=false;
        int count = 0;
...
{
    spdlog::warn("COMPARING THE SIMULATION RESULTS FOR SEVERAL REFINE FRACTIONS. THIS IS ONLY FOR EVALUATION. FOR ACTUAL USE OF THE ALGORITHM, CALL updateSourceProbability() DIRECTLY");
    Utils::Time::Clock clock;
    ...
    auto logResult = [&](float fraction){
        ...
        outfile<<fmt::format("{} -> time:{}, KLD:{}\n", fraction, Utils::Time::toSeconds(clock.now()-start), Utils::KLD(fullprob, testedprob) );
