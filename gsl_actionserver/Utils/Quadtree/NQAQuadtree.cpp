...
namespace NQA{
    ...
    bool Node::subdivide(){
        ...
        // unlike a proper quadtree, we can have a node that only has two children, rather than four. This can happen if we have a non-square cell that reaches the minimum size on one of the dimensions
    ...
    std::vector<Node> NQAQuadtree::fusedLeaves(int maxSize)
    {
...

        {
            ...
            while(to_be_evaluated.size()!= 0)
            {
                ...
                {
                    //if(can be fused)
                    if((current->origin.x == neighbour->origin.x && current->size.x == neighbour->size.x && (current->size.y + neighbour->size.y)<=maxSize )
                        || (current->origin.y == neighbour->origin.y && current->size.y == neighbour->size.y && (current->size.x + neighbour->size.x)<=maxSize) )
                    {
                        Vector2Int origin; Vector2Int size;
