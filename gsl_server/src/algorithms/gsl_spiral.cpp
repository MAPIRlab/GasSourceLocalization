...
SpiralSearcher::SpiralSearcher(std::shared_ptr<rclcpp::Node> _node) : GSLAlgorithm(_node)
{
}

void SpiralSearcher::initialize()
{
...
void SpiralSearcher::declareParameters()
{
    ...
    minPI = getParam<double>("minPI", 0.1);
    stop_and_measure_time = getParam<double>("stop_and_measure_time", 1.0);

    initStep = getParam<double>("initial_step", 0.6);
    step_increment = getParam<double>("step_increment", 0.3);
    Kmu = getParam<double>("Kmu", 0.5);
    Kp = getParam<double>("Kp", 1);
    intervalLength = getParam<double>("intervalLength", 0.5);

    gas_present_thr = getParam<double>("gas_present_thr", 1);
}
...
{
    if (verbose)
        spdlog::info("[SPIRAL_SEARCH] - {} - Got the map of the environment!", __FUNCTION__);
...
void SpiralSearcher::getGasObservations()
{
    if ((node->now() - time_stopped).seconds() > stop_and_measure_time)
    {
        ...
        if (PI > previousPI)
        {
            ...
            resetSpiral();
            if (verbose)
                spdlog::warn("[SPIRAL_SEARCH] NEW SPIRAL --> Proximity Index:{:.4}", PI);
        }
        else
        {
            // Nothing
            ...
            current_state = SPIRAL_state::SPIRAL;
            if (verbose)
                spdlog::warn("[SPIRAL_SEARCH] CONTINUE --> Proximity Index:{:.4}", PI);
            if (consecutive_misses > 3)
            {
                previousPI = previousPI / 2.0;
                if (verbose)
                    spdlog::warn("[SPIRAL_SEARCH] PI reduced to:{:.4}", previousPI);
