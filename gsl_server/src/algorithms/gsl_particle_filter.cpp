...
{
    if (x < map_.info.origin.position.x || x >= map_.info.origin.position.x + map_.info.width * map_.info.resolution ||
        y < map_.info.origin.position.y || y >= map_.info.origin.position.y + map_.info.height * map_.info.resolution)
    {
        return false;
    }
