...
namespace PMFS
{
            ...
            for (int b = 0; b < cells[0].size(); b++)
            {
                if (cells[a][b].free)
                {
                    ...
                    std_msgs::msg::ColorRGBA col_hit =
                        valueToColor(Utils::logOddsToProbability(cells[a][b].hitProbability.logOdds), settings.visualization.hitLimits.x,
                    ...
                    std_msgs::msg::ColorRGBA colorConfidence =
                        valueToColor(cells[a][b].hitProbability.confidence, 0, 1, Utils::valueColorMode::Linear);
    ...
    void PMFS_GSL::showDebugInfo()
    {
        ...
        for (int a = 0; a < cells.size(); a++)
        {
            for (int b = 0; b < cells[0].size(); b++)
            {
                ...
                varianceColor = Utils::valueToColor(simulations.varianceOfHitProb[a][b] * (1 - cells[a][b].hitProbability.confidence), minVar, maxVar,
    ...
    void PMFS_GSL::debugMapSegmentation()
    {
        ...
        for (int i = 0; i < simulations.QTleaves.size(); i++)
        {
            ...
            auto coords = indexToCoordinates(leaf->origin.x, leaf->origin.y, false) + (worldSpaceScale * 0.5);
    ...
    {
#ifdef USE_GUI
        AMENT_IMGUI::setup(fmt::format("{}/resources/imgui.ini", ament_index_cpp::get_package_share_directory("gsl_server")).c_str(), "PMFS", 900,
                           600);
        GSLIMPLOT::setup();
